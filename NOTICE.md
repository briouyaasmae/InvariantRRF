# Copyright and license scope

Copyright (c) 2026 Asmae Briouya and contributors.

The original source code, scripts, and Jupyter notebooks in this repository are
licensed under the GNU General Public License as published by the Free Software
Foundation, either version 3 of the License, or (at your option) any later
version (GPL-3.0-or-later). See `LICENSE` for the full license text.

The GPL grant applies only to original software and notebook content for which
the repository authors hold the rights to license.

Third-party datasets, pretrained models, libraries, benchmark resources, and
other external materials are not relicensed by this repository. They remain
subject to their own licenses, terms, and attribution requirements.

The research manuscript is intentionally not included in this repository.
Publication text, tables, figures, and other manuscript content are not
licensed by this repository unless explicitly stated elsewhere by the
copyright holders.

If you use this implementation in academic work, please cite the associated
InvariantFusion research paper and this software repository where appropriate.
